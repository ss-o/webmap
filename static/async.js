...
$(document).ready(function () {
  ...
  $.get("https://api.github.com/repos/Rev3rseSecurity/WebMap").done(function (
    d
  ) {
  ...
  $.get("/report/api/" + address + "/" + portid + "/").done(function (data) {
    ...
    $("#modalbody").append(
      "<br><b>Raw Output:</b><br><pre>" +
        $("<span/>").text(JSON.stringify(data, null, 4)).html() +
        "</pre>"
    );
    $("#modal1").modal("open");
  });
...
function removeLabel(type, hashstr, i) {
  $.get("/report/api/rmlabel/" + type + "/" + hashstr + "/").done(function (
    data
  ) {
